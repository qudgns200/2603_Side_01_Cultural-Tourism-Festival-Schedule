...
기존에는 `index.html`이 `list.html`을 고정 높이 iframe(800px / 모바일 600px)으로 감싸고 있었다. 카드가 많으면 내부 스크롤로 처리되고, 부모와 자식 사이에 필터 상태를 주고받을 방법이 없어 모바일 우선 반응형의 걸림돌이었다. `list.html`을 `index.html`로 합치고 삭제했다.

iframe을 걷어내면서 드러난 문제도 함께 처리했다. 흰 카드 표면을 만들던 것이 iframe 래퍼였기 때문에, 그대로 두면 카드가 핑크·퍼플 그라디언트 위에 놓여 대비가 무너진다. 히어로에는 그라디언트를 유지하되 결과 영역에 `#fafafa` 시트를 깔아 기존 Vibrant Festival 룩을 보존했다. 실제 오버플로를 감추고 있던 `overflow-x: hidden`도 제거하고 근본 원인을 고쳤다.

**주요 변경**

- **단일 페이지 구성** — 헤더 / 히어로(주말 날짜 + 지역 선택) / 인기 행사 레일 / 카테고리 칩 / 결과 목록 / 상세 모달
- **상태 관리** — `setState()` → `applyFilters()` → `render()` 단일 경로. 필터를 조작해도 **API 재호출이 0회**임을 브라우저 테스트로 확인했다 (명세서 §4.2 원칙 2)
- **카드 UI** — `<template>` + `cloneNode` + `textContent`로만 구성. 이스케이프 규율이 아니라 구조로 XSS를 차단한다. 이미지 `loading="lazy"`, 고정 3:2 비율로 CLS 방지, 로드 실패 시 플레이스홀더 대체
- **필터** — 지역 16개 · 날짜(전체/토/일) · 무료 · 카테고리 6종(OR 조건) · 찜한 행사
- **빈 결과 구분** — "필터가 과함"과 "데이터 자체가 없음"을 다른 문구로 처리하고, 전자에는 걸려 있는 필터에 해당하는 완화 버튼만 조건부로 노출한다
- **오류 상태** — 기술적 메시지를 노출하지 않고 재시도 버튼을 제공한다. 쿼터 초과만 별도 문구
- **로딩** — 실제 카드 크기에 맞춘 스켈레톤 6장
- **상세 모달** — 네이티브 `<dialog>` + `showModal()`. 포커스 트랩·Esc 닫기·포커스 복원이 기본 제공된다
- **찜하기** — LocalStorage `favoriteFestivals`. id가 아니라 스냅샷 객체를 저장한다. 지난주에 찜한 행사는 이번 주말 목록에 없어 id만으로는 아무것도 그릴 수 없기 때문이다
- **접근성** — 건너뛰기 링크, `aria-live` 결과 개수, 지역 select 라벨, 칩 `aria-pressed` + 체크 표시(색만으로 상태를 알리지 않음), 전역 `:focus-visible`, 터치 타깃 44px, `prefers-reduced-motion` 대응. `--primary`를 `#f5576c` → `#c2334a`로 조정해 흰 배경 대비 WCAG AA를 만족시켰다
- **SEO/OG** — title·description·canonical·Open Graph·theme-color, 인라인 SVG 파비콘

**브라우저 테스트에서 잡은 버그 4건**

Playwright로 실제 렌더링을 확인해 다음을 찾아 고쳤다. 정적 검사만으로는 드러나지 않는 것들이다.

1. `.popular { color: #fff }`가 레일 카드로 상속되어 **흰 배경에 흰 제목**이 되던 문제. 위치·날짜는 색을 명시해 두어 정상 보였기 때문에 더 눈에 띄지 않았다
2. `.card-noimg`의 `display: flex`가 `[hidden]`의 `display: none`을 이겨 **이미지 위에 플레이스홀더가 겹치던** 문제
3. 세로형 포스터가 `aspect-ratio`를 무시하고 부모 높이를 밀어내 **카드마다 이미지 높이가 달라지던** 문제 (비율 1.50이어야 할 것이 0.71). img를 `position: absolute`로 흐름에서 빼내 해결
4. 최상단 `* { margin: 0 }` 리셋이 `<dialog>`의 기본 `margin: auto`를 지워 **모달이 좌측에 붙던** 문제

네 건 모두 회귀 테스트를 추가했다. 최종 **43개 항목 전체 통과**(렌더링·필터·찜하기·모달·접근성·모바일·대비·비율).

**문서 갱신**

- `API.md` — 전면 재작성. 클라이언트/상위 API 명세, 오류 코드표, **실측 필드 채움률**, 지역·태그·무료 판정 규칙, 캐싱 전략
- `CLAUDE.md` — 2.0 구조로 갱신. 키 인코딩·쿼터·subrequest 한도·좌표 순서·KST 보정 등 실측으로 확인된 함정을 별도 절로 정리
- `README.md` — 주말 발견 서비스 기준으로 재작성

**주요 변경 파일**
- `public/index.html` (전면 재작성 — 단일 페이지)
- `public/css/style.css` (신규)
- `public/js/app.js` (신규)
- `public/list.html` (삭제 — 통합)
- `API.md`, `CLAUDE.md`, `README.md` (재작성)
